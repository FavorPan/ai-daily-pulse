  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <div className="lg:col-span-1">
        <div className="bg-surface p-4 rounded-xl space-y-2 sticky top-6 border border-border">
      ...
      <div className="lg:col-span-3">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((item) => (
              <ProjectCard key={`${item.digestDate}-${item.id}`} item={item} showDate={globalSearch} />
            ))}
          </div>
        )}
      </div>

...
## Prompt 1：评分筛选（每篇文章执行一次，配置的 LLM 模型）
...
keep 规则：score >= 5 且 topic != 无关 时为 true（GitHub Trending 来源 score >= 4 即为 true）。
注：阈值已针对 V4-Flash 模型校准，该模型在相同输入上比 Haiku 低约 1.5 分。
只输出 JSON，不要其他文字。
```

---

## Prompt 2：摘要生成（仅对保留文章执行，配置的 LLM 模型）
...
## Prompt 3：去重（对所有保留文章执行一次，Claude Haiku）
